...
public class CollectionConcepts {
	...
	public static void main(String[] args) {
		...
		// 3. ArrayList implements a toString method that will list our objects in
		// turn.
...
"***5. one thing we can do with an iterator, that we can't do with a for each loop! we can remove elements with an iterator, i.e");
		...
		while (secondProductIterator.hasNext()) {
			...
			if (product.getWeight() > 20) {
				...
				secondProductIterator.remove();  //remove product from collection if weight is less than 20kg
			}
